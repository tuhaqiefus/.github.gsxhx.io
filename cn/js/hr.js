document.addEventListener('DOMContentLoaded', function() {
    ...
    // 如果mainHeader不存在，跳过导航相关功能
    if (!mainHeader) {
        console.log('mainHeader not found, skipping navigation functionality');
    }
    ...
    function isMouseOverHeaderAndDropdowns() {
        if (mainHeader && mainHeader.matches(':hover')) {
            return true;
        }
    ...
    function initCarousel() {
        if (!carousel || items.length === 0) {
            console.log('Carousel not found or no items');
            return;
        }
